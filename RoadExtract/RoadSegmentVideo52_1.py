...
def run_yolo_segmentation_on_video(model, video_path, conf=0.5, display=True, save_path=None, transparency=0.5):
    ...
    if save_path:
        ...
        out2 = cv2.VideoWriter("output\\videoNotSeg.mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps,
    ...
    while cap.isOpened():
        ...
        if not doesSeg:
            cv2.imwrite(f"imgDir2/{ImgC + 1:05d}.png", frame)
            ImgC += 1
            if save_path:
                out2.write(frame)
...
for model_number in range(18, 19):
    ...
    video_indices = [27, 3, 4, 5, 7, 30, 32, 33, 36, 39, 40, 41, 43, 44, 45, 46, 47, 28, 29]
        ...
        if i in [8, 27, 30]:
            try:
                ...
                total_frames = run_yolo_segmentation_on_video(
                    ...
                    display=True,
                    transparency=0.5,
                    save_path="output1\\videoNotSeg2.mp4",
                    video_path=f"output1\\videoNotSeg.mp4"
                )
